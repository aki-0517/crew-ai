...
export default function ChatInput({
  ...
  return (
    <form
      ...
      className={`overflow-hidden rounded-lg border transition-colors ${disabled ? 'cursor-not-allowed' : 'focus-within:border-sky-700 hover:border-sky-700'} ${isDarkMode ? 'border-slate-700' : ''}`}
        ...
        <div
          ...
          {showStopButton ? (
            ...
            <button
              ...
              className={`rounded-md bg-sky-700 px-3 py-1 text-white transition-colors hover:enabled:bg-sky-800 ${isSendButtonDisabled ? 'cursor-not-allowed opacity-50' : ''}`}>
              {t('chat_buttons_send')}
            </button>
          )}
        </div>
